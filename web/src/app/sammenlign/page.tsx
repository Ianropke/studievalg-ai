    ...
    <div className="min-h-screen bg-[#F7F8FA] text-[#12172B] antialiased">
      ...
      <main className="max-w-5xl mx-auto px-6 py-10 space-y-8">
        ...
        <div className="bg-[#FFFFFF] p-6 rounded-2xl border border-[#E7E9EF] card-shadow flex flex-col sm:flex-row items-center gap-8">
          <div className="shrink-0 flex flex-col items-center"><MultiTriangleRadar programs={selectedPrograms} /><span className="text-[10px] font-semibold text-[#8891A3]">Overlappende Trekant-Profil</span></div>
          <div className="flex-1 space-y-4 w-full"><h3 className="text-sm font-bold text-[#12172B]">Sammenligningskonfiguration</h3><div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            {selectedPrograms.map((prog, idx) => { const style = colors[idx % colors.length]; const slug = createProgramSlug(prog); return <div key={slug} className={`p-3 rounded-xl border ${style.border} ${style.bg} space-y-2 relative`}><div className="flex justify-between items-center"><span className={`text-[10px] font-bold ${style.text}`}>{style.badge}</span>{selectedPrograms.length > 1 && <button onClick={() => removeProgram(idx)} className="text-xs text-[#8891A3] hover:text-[#EF4444] font-bold px-1" title="Fjern fra sammenligning">✕</button>}</div><Link href={`/uddannelse/${slug}`} className="font-bold text-xs text-[#12172B] hover:underline block truncate">{prog.udbud_titel}</Link><p className="text-[11px] text-[#545D71] truncate">{prog.institution || prog.institution_navn}</p></div>; })}
            {selectedPrograms.length < 3 && <button onClick={() => setShowSearchModal(true)} className="p-4 rounded-xl border border-dashed border-[#D8DBE4] hover:border-[#2563EB] bg-[#F7F8FA] hover:bg-[#EFF6FF] text-[#2563EB] font-bold text-xs transition flex flex-col items-center justify-center gap-1 h-full min-h-[90px]"><span>+ Tilføj uddannelse</span><span className="text-[10px] text-[#8891A3] font-normal">(Op til 3 samtidig)</span></button>}
          </div></div>
        </div>
