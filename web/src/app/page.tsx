...
export default function Dashboard() {
  ...
  const matchedPrograms = useMemo(() => {
    ...
    let list = allPrograms.map((prog) => {
      ...
      const jobScore = enriched.labour_demand;
      const salScore = enriched.salary_growth;
      ...
      const aiPhrase = enriched.ranking_eligible.ai
        ? `det modelbaserede AI-resiliensindeks er ${robustScore}/100`
        : "AI-dimensionen er neutraliseret, fordi der ikke er en defensibel program-specifik O*NET 31.0-kobling";
      const jobPhrase = enriched.ranking_eligible.job
        ? `den dokumenterede jobindikator er ${jobScore}/100`
        : "jobdimensionen er neutraliseret, fordi program-specifik arbejdsmarkedsprovenance mangler";

      let whyText = "";
      if (kvNum !== null) {
        if (meetsGpa) {
          whyText = `Med et snit på ${gpa.toFixed(1)} opfylder du den seneste Kvote 1-kvotient på ${kvNum}. ${aiPhrase}; ${jobPhrase}.`;
        } else {
          whyText = `Den seneste Kvote 1-kvotient var ${kvNum}, så et snit på ${gpa.toFixed(1)} ligger under den observerede grænse. ${aiPhrase}; ${jobPhrase}.`;
        }
      } else {
        whyText = `KOT-data angiver “Alle optaget” for den seneste optagelse. ${aiPhrase}; ${jobPhrase}.`;
      }
  ...
  const preferenceModeLabel = preferenceMode === "priority" ? "Prioritering" : `Minimumskrav · ${requirementMatchMode === "all" ? "alle aktive krav" : "mindst ét aktivt krav"}`;
  const preferenceHelpText = preferenceMode === "priority"
    ? "Sliderne bestemmer vægtene for de tilgængelige modelindikatorer. Neutraliserede 50-værdier skaber ikke programforskelle."
    : "Sliderne er minimumsniveauer for modelindikatorerne. En neutraliseret 50-værdi er manglende evidence, ikke en måling.";
      ...
      <main className="max-w-5xl mx-auto px-6 py-10 space-y-10">
        <div className="text-center max-w-3xl mx-auto space-y-3">
          ...
          <p className="text-sm text-[#545D71] max-w-lg mx-auto font-normal leading-relaxed">
            Sammenlign uddannelser med observerede KOT-optagelsesdata og tydeligt markerede modelindikatorer. AI-, job- og løndimensioner differentierer kun, når den relevante programmevidence er tilstrækkelig; ellers er de neutrale placeholders.
          </p>
        </div>
        ...
        <div className="bg-[#FFFFFF] border border-[#E7E9EF] rounded-xl p-6 card-shadow space-y-6 max-w-3xl mx-auto">
          ...
          <div className="rounded-lg border border-[#FDE68A] bg-[#FFFBEB] px-3 py-2 text-xs text-[#92400E]">
            KOT/adgangstal er observerede admissionsdata. AI-resiliens er et O*NET-baseret model/crosswalkindeks, ikke en automatiseringssandsynlighed. Job- og lønindikatorer uden dokumenteret programkilde samt legacy AI-input neutraliseres til 50 i klientrangeringen; 50 betyder “ingen differentierende evidence”, ikke en vurdering af den sande værdi.
          ...
          <div data-testid="preference-mode-panel" className="rounded-lg border border-[#D8DBE4] bg-[#FFFFFF] px-4 py-3 space-y-3">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
              ...
              <label className="text-xs font-semibold text-[#12172B] flex items-center gap-2">
                ...
                <select id="preference-mode" value={preferenceMode} onChange={(e) => setPreferenceMode(e.target.value as PreferenceMode)} className="bg-[#F7F8FA] border border-[#D8DBE4] rounded-lg px-3 py-2 text-xs text-[#12172B] focus:outline-none focus:ring-2 focus:ring-[#2563EB] focus:border-[#2563EB]">
            ...
            {preferenceMode === "requirements" && (
              <div className="flex flex-col sm:flex-row sm:items-center gap-2 pt-2 border-t border-[#E7E9EF]">
                ...
                <select id="requirement-match-mode" value={requirementMatchMode} onChange={(e) => setRequirementMatchMode(e.target.value as RequirementMatchMode)} className="bg-[#F7F8FA] border border-[#D8DBE4] rounded-lg px-3 py-2 text-xs text-[#12172B] focus:outline-none focus:ring-2 focus:ring-[#2563EB] focus:border-[#2563EB]">
          ...
          <div className="space-y-4">
            <div className="flex justify-between items-center">
              <span className="text-xs font-bold text-[#545D71] uppercase tracking-wider flex items-center gap-1.5"><SlidersIcon /> {preferenceMode === "priority" ? "Hvad betyder mest for dig?" : "Hvilke minimumskrav skal opfyldes?"}</span>
              <span aria-live="polite" className="text-[11px] text-[#0B7A57] font-mono-data font-semibold"><span data-testid="weight-summary">{preferenceMode === "priority" ? "Aktuelle prioriteter" : preferenceModeLabel} · {preferenceSummary}</span></span>
            </div>
        ...
        <div className="space-y-4 max-w-3xl mx-auto">
          ...
          {loading ? (
            ...
            <div className="space-y-4" data-testid="ranked-program-results">
              <p aria-live="polite" className="text-xs text-[#545D71]">Rangeringen opdateres ved hvert slidertræk. {preferenceMode === "priority" ? "Aktuelle prioriteter" : "Aktuelle minimumskrav"}: <span className="font-semibold text-[#12172B]">{preferenceSummary}</span></p>
                  ...
                  <article key={prog.kot_nr} data-testid="program-card" data-program-id={prog.kot_nr} className="border border-[#E7E9EF] bg-[#FFFFFF] rounded-xl p-5 card-shadow hover:border-[#D8DBE4] transition space-y-4">
                    ...
                    {isExpanded && (
                      <div className="pt-4 border-t border-[#E7E9EF] space-y-4 text-xs">
                        ...
                        <div className="space-y-2 pt-2 border-t border-[#E7E9EF]"><span className="font-bold text-[#12172B] uppercase tracking-wider text-[11px] block">Modelstatus og evidenskilder</span><ScoreDisclosure scores={prog.scoreDetails!} /><EvidenceList evidence={prog.rag_evidence} /></div>
                      </div>
                    )}
                  </article>
                );
            ...
            <div className="bg-[#FFFFFF] border border-[#E7E9EF] rounded-xl p-8 text-center space-y-3 card-shadow"><h3 className="text-base font-bold text-[#12172B]">Ingen matchende uddannelser fundet</h3><p className="text-xs text-[#545D71]">{preferenceMode === "requirements" ? "Ingen uddannelser opfylder den valgte kravlogik. Husk at en neutral 50-værdi betyder manglende programmevidence, ikke et dokumenteret minimumsniveau." : "Prøv at vælge et andet universitet eller ryd dit søgefelt."}</p><button onClick={() => { setSearchQuery(""); setSelectedUniversity("all"); }} className="px-4 py-2 bg-[#12172B] text-[#FFFFFF] rounded-lg text-xs font-semibold hover:bg-[#545D71] transition focus:outline-none focus:ring-2 focus:ring-[#2563EB] focus:ring-offset-2">Nulstil søgning og vis alle</button></div>
          )}
      ...
      {showShareModal && (
        <div className="fixed inset-0 z-50 bg-[#12172B]/60 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-[#FFFFFF] border border-[#E7E9EF] rounded-2xl p-6 sm:p-8 max-w-lg w-full card-shadow space-y-6 relative animate-in fade-in zoom-in-95 duration-150 text-left">
            <button onClick={() => setShowShareModal(false)} className="absolute top-4 right-4 w-8 h-8 rounded-full bg-[#F7F8FA] hover:bg-[#E7E9EF] text-[#545D71] font-bold flex items-center justify-center transition" aria-label="Luk modal">✕</button>
            <div className="space-y-2"><div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-semibold bg-[#E3F6EE] text-[#0B7A57] border border-[#0F9D6E]/20"><span>🎯 Dit Personlige Uddannelsesmatch</span></div><h3 className="text-2xl font-bold text-[#12172B] tracking-tight font-display">Del dit resultat</h3><p className="text-xs text-[#545D71]">Generér et direkte link eller kopiér dit top-match. Modelscoren kan indeholde neutrale placeholders for manglende programmevidence.</p></div>
            {matchedPrograms.length > 0 && (
              <div className="bg-gradient-to-br from-[#12172B] to-[#1E293B] text-[#FFFFFF] rounded-xl p-5 space-y-4 shadow-md">
                <div className="flex justify-between items-center text-[11px] text-[#A1A1AA] border-b border-[#3F3F46] pb-3"><span className="font-bold text-[#38BDF8]">UDDANNELSESINDSIGT.COM</span><span className="bg-[#0F9D6E] text-white font-mono-data px-2 py-0.5 rounded text-[10px] font-bold">{matchedPrograms[0].matchScore}% MODELSCORE</span></div>
                <div className="space-y-1"><span className="text-xs font-bold text-[#10B981] uppercase tracking-wider">#1 Top Anbefaling</span><h4 className="text-xl font-bold tracking-tight text-white font-display">{matchedPrograms[0].udbud_titel}</h4><p className="text-xs text-[#D4D4D8]">{matchedPrograms[0].institution} • KOT {matchedPrograms[0].kot_nr}</p></div>
                <div className="bg-[#27272A] p-3 rounded-lg flex flex-wrap justify-between text-[11px] text-[#A1A1AA] gap-2 font-mono-data"><span>Snit: {gpa.toFixed(1)}</span><span>AI-vægt: {aiRobustnessWeight}%</span><span>Job: {jobOpportunitiesWeight}%</span><span>Løn: {salaryWeight}%</span></div>
              </div>
            )}
            <div className="space-y-3">
              <button onClick={() => { const shareUrl = createShareUrl(); navigator.clipboard.writeText(shareUrl).then(() => { track("match_share", { method: "copy_link", mode: preferenceMode }); setCopiedToast(true); setTimeout(() => setCopiedToast(false), 3000); }).catch(() => {}); }} className="w-full py-3 bg-[#12172B] hover:bg-[#1E293B] text-[#FFFFFF] font-bold rounded-xl text-xs flex items-center justify-center gap-2 transition card-shadow focus:outline-none focus:ring-2 focus:ring-[#2563EB]"><span>📋</span><span>Kopiér direkte match-link</span></button>
              {typeof navigator !== "undefined" && "share" in navigator && (
                <button onClick={() => { const shareUrl = createShareUrl(); const topTitle = matchedPrograms.length > 0 ? matchedPrograms[0].udbud_titel : "Uddannelse"; const topScore = matchedPrograms.length > 0 ? String(matchedPrograms[0].matchScore) : null; navigator.share({ title: topScore ? `Min modelscore: ${topTitle} (${topScore}%)` : "Mit uddannelsesmatch på Uddannelsesindsigt", text: topScore ? `Min modelscore er ${topScore}% for ${topTitle} på Uddannelsesindsigt. Scoren er beslutningsstøtte og kan indeholde neutrale placeholders for manglende programmevidence.` : "Jeg har endnu ikke et beregnet uddannelsesmatch. Se mit match på Uddannelsesindsigt:", url: shareUrl }).then(() => { track("match_share", { method: "native", mode: preferenceMode }); }).catch(() => {}); }} className="w-full py-3 bg-[#E3F6EE] hover:bg-[#D2F1E4] text-[#0B7A57] border border-[#0F9D6E]/30 font-bold rounded-xl text-xs flex items-center justify-center gap-2 transition focus:outline-none focus:ring-2 focus:ring-[#0F9D6E]"><span>📱</span><span>Del via apps</span></button>
              )}
              <button onClick={() => { const topTitle = matchedPrograms.length > 0 ? matchedPrograms[0].udbud_titel : "Uddannelse"; const topScore = matchedPrograms.length > 0 ? String(matchedPrograms[0].matchScore) : null; const shareUrl = createShareUrl(); const storyText = topScore ? `🎯 Min top-modelscore er ${topTitle} (${topScore}%) på Uddannelsesindsigt. Scoren er beslutningsstøtte og ikke en prognose.\n\nFind dit eget match her: ${shareUrl}` : `Jeg har endnu ikke et beregnet uddannelsesmatch på Uddannelsesindsigt.\n\nFind dit eget match her: ${shareUrl}`; navigator.clipboard.writeText(storyText).then(() => { track("match_share", { method: "copy_text", mode: preferenceMode }); setCopiedToast(true); setTimeout(() => setCopiedToast(false), 3000); }).catch(() => {}); }} className="w-full py-3 bg-[#F7F8FA] hover:bg-[#E7E9EF] text-[#12172B] border border-[#E7E9EF] font-bold rounded-xl text-xs flex items-center justify-center gap-2 transition focus:outline-none focus:ring-2 focus:ring-[#2563EB]"><span>💬</span><span>Kopiér teksten til Socials / Chat</span></button>
            </div>
