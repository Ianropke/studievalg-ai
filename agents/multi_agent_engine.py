"""Deterministic evidence-aware recommendation and analytics pipeline."""
...
if CATALOG_PATH.exists():
    try:
        ...
        sys.stderr.write(f"Warning loading catalog: {exc}\n")
...
def compute_canonical_ai_resilience(auto_risk: float, aug_pot: float) -> float:
    """Return the canonical model/crosswalk AI-resilience index in [0.1, 1.0]."""
    risk = max(0.0, min(1.0, float(auto_risk)))
    aug = max(0.0, min(1.0, float(aug_pot)))
    resilience = (0.75 * (1.0 - risk)) + (0.25 * aug)
    return round(max(0.1, min(1.0, resilience)), 3)


def _required_unit_interval(value, metric_name: str) -> float:
    """Parse a required model input without inventing a fallback value."""
    if value is None:
        raise ValueError(f"{metric_name} is missing")
    try:
        parsed = float(value)
    except (TypeError, ValueError) as exc:
        raise ValueError(f"{metric_name} is not numeric") from exc
...
class MultiAgentEngine:
    ...
    def _reasoning_agent(self, plan, retrieved_data, evidence):
        ...
        for p in retrieved_data["profiles"]:
            kot = str(p["kot_nr"])
            try:
                auto_risk = _required_unit_interval(p.get("automation_risk"), "automation_risk")
                aug_pot = _required_unit_interval(p.get("augmentation_potential"), "augmentation_potential")
                lab_dem = _required_unit_interval(p.get("labour_demand"), "labour_demand")
                sal_gro = _required_unit_interval(p.get("salary_growth"), "salary_growth")
            except ValueError:
                # The strict analytics profile is expected to be complete. A corrupt or
                # incomplete row is excluded instead of receiving plausible fallback numbers.
                continue
            ...
            if interest_fit >= 0.80:
                top_factors.append("Stærkt deterministisk fagligt søgematch")
            if ai_resilience >= 0.75:
                top_factors.append("Højt modelbaseret AI-resiliensindeks (occupational crosswalk)")
            if lab_dem >= 0.80:
                top_factors.append("Høj afledt arbejdsmarkedsindikator i det dokumenterede datagrundlag")
            if sal_gro >= 0.80:
                top_factors.append("Høj afledt lønvækstindikator i det dokumenterede datagrundlag")
            if loc_fit == 1.0 and preferred_loc:
                top_factors.append(f"Match på ønsket studieby ({preferred_loc.capitalize()})")

            if auto_risk >= 0.35:
                main_risks.append(
                    f"Højere modelestimeret opgaveeksponering i occupational crosswalk ({round(auto_risk * 100)} på 0–100-indekset)"
                )
            if lab_dem < 0.60:
                main_risks.append("Lavere afledt arbejdsmarkedsindikator i det dokumenterede datagrundlag")
            ...
            scored_programs.append({
                ...
                "metric_status": {
                    "ai_resilience": "CROSSWALK_MODEL",
                    "labour_demand": "DERIVED_FROM_DOCUMENTED_EDUCATION_DATA",
                    "salary_growth": "DERIVED_FROM_DOCUMENTED_EDUCATION_DATA",
                    "interest_fit": "MODEL_HEURISTIC",
                    "location_fit": loc_source,
                },
                "score_components": {
                ...
                "top_positive_factors": top_factors if top_factors else ["Ingen enkelt modelkomponent dominerer det samlede match"],
                "main_risks": main_risks if main_risks else ["Ingen modelindikator overskrider de valgte risikotærskler"],
    ...
    def _counterargument_agent(self, top_program):
        if not top_program:
            return "Ingen kandidatuddannelse at analysere for modelusikkerhed."
        title = top_program.get("udbud_titel", "uddannelsen")
        risk_pct = top_program.get("automation_risk_pct")
        if not risk_pct:
            return (
                f"Modelbaseret forbehold for {title}: Opgaveeksponering kunne ikke fastsættes fra det dokumenterede modelinput. "
                "Der gives derfor ingen numerisk fallbackvurdering."
            )
        return (
            f"Modelbaseret forbehold for {title}: Occupational crosswalk/modellen giver et opgaveeksponeringsindeks på {risk_pct}. "
            "Tallet er ikke en automatiseringssandsynlighed eller en prognose for dansk beskæftigelse; det beskriver kun modellens nuværende opgaveeksponeringssignal."
        )
    ...
    def _data_validator_agent(self, scored_programs):
        ...
        for p in scored_programs:
            kot = p.get("kot_nr", "")
            missing = [key for key in required_numeric if p.get(key) is None]
            if missing:
                rejected_programs.append(p)
                rejection_reasons[kot] = f"Missing required numerical metrics: {', '.join(missing)}"
                continue

            try:
                values = {key: float(p[key]) for key in required_numeric}
            except (TypeError, ValueError):
                rejected_programs.append(p)
                rejection_reasons[kot] = "Required numerical metric is not numeric"
                continue
    ...
    def _citation_agent(self, evidence, candidate_program):
        ...
        for c in evidence["evidence_chunks"]:
            ...
            relevance_score = min(0.95, round(0.30 + (0.25 * matching_tokens), 2))
            authority = classify_source_authority(source_title, source_url)
            supports = authority != "UNKNOWN" and relevance_score >= 0.70

            if supports:
                citations.append({
                    "claim_id": f"claim-{candidate_program.get('kot_nr', 'unknown')}",
                    "source": source_title,
                    "url": source_url,
                    "quote": c["chunk_text"],
    ...
    def _ui_formatter_agent(self, validation_payload, counterargument, evidence, user_query):
        ...
        return {
            ...
            "explainability": {
                "user_model_matching": "Deterministisk 5-faktor model med eksplicitte komponentvægte",
                "knowledge_graph_chain": "Education -> Course -> Task -> DISCO-08 -> Industry",
                "register_data_verification": "Dokumenteret KOT/UFM/DST- og uddannelsesgruppekildekæde, hvor den konkrete profilbygger har bestået",
            },
        }
    ...
    def run_pipeline(self, user_query, user_profile=None):
        ...
        return self._ui_formatter_agent(validation_payload, counterargument, evidence, user_query)
