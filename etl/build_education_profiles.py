...
REQUIRED_LABOUR = {
    "kot_nr", "labour_period", "employment_rate", "unemployment_rate",
    "salary_median", "salary_5y_growth", "labour_source", "labour_dataset",
    "labour_source_url", "salary_source", "salary_dataset", "salary_period",
    "salary_source_url"
}
...
def _read_csv(path: Path, required: set[str], name: str) -> pd.DataFrame:
    if not path.exists():
        raise FileNotFoundError(
            f"{path} is missing. Add the official source export described in data/DATA_SOURCE_CONTRACT.md; the pipeline will not fabricate values."
        )
...
def build_profiles() -> dict:
    ...
    try:
        ...
        required_join = ["disco08_code", "employment_rate", "unemployment_rate", "salary_5y_growth", "automation_risk", "augmentation_potential", "salary_source", "salary_dataset", "salary_period"]
        ...
        merged["labour_demand"] = (0.7 * _percentile(merged["employment_rate"], True) + 0.3 * _percentile(merged["unemployment_rate"], False)).clip(0, 1)
        ...
        conn.execute("""
            CREATE TABLE education_profile_scores (
                kot_nr VARCHAR PRIMARY KEY, udbud_titel VARCHAR, disco08_code VARCHAR NOT NULL, disco_titel VARCHAR,
                ...
                international_mobility DOUBLE, future_uncertainty DOUBLE,
        ...
        for _, r in merged.iterrows():
            rows.append([
                ...
                float(r.automation_risk), float(r.augmentation_potential), float(r.labour_demand), float(r.salary_growth), None, None,
    ...
    result = {
        "schema_version": "1.3", "programme_count": int(len(merged)),
        "sources": {"mapping": str(MAPPING_PATH.relative_to(BASE_DIR)), "labour_market": str(LABOUR_PATH.relative_to(BASE_DIR)), "ai_occupation": str(AI_PATH.relative_to(BASE_DIR))},
        ...
        "epistemic_status": {"labour_demand": "DERIVED", "salary_growth": "DERIVED", "automation_risk": "CROSSWALK_OR_MODEL", "augmentation_potential": "CROSSWALK_OR_MODEL"},
    }
