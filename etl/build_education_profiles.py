    ...
    for code, group in salary.groupby("education_code", sort=False):
        ...
        for year, current in lookup.items():
            if year - 5 in lookup and lookup[year - 5] > 0:
                records.append({"education_code": code, "period": str(year), "salary_5y_growth": current / lookup[year - 5] - 1.0, "salary_value": current})
    if not records:
...
def build_profiles() -> dict:
    ...
    for df, keys, name in [(education, ["kot_nr"], "education"), (disco, ["kot_nr"], "disco")]:
    ...
    latest_labour = labour.assign(period_num=pd.to_numeric(labour["period"], errors="raise")).sort_values("period_num").drop_duplicates("education_code", keep="last")
    ...
    try:
        ...
        merged = merged.merge(disco, on="kot_nr", how="left", validate="one_to_one", suffixes=("", "_disco"))
        merged = merged.merge(ai, on="disco08_code", how="left", validate="many_to_one", suffixes=("", "_ai"))

        required = ["education_code", "employment_rate", "unemployment_rate", "salary_5y_growth", "disco08_code", "automation_risk", "augmentation_potential", "mapping_confidence", "mapping_confidence_disco", "mapping_confidence_ai"]
        ...
        conn.execute("""CREATE TABLE education_profile_scores (
            ...
            education_code VARCHAR NOT NULL, education_title VARCHAR NOT NULL, disco08_code VARCHAR NOT NULL,
            automation_risk DOUBLE NOT NULL, augmentation_potential DOUBLE NOT NULL, labour_demand DOUBLE NOT NULL, salary_growth DOUBLE NOT NULL,
            employment_rate DOUBLE NOT NULL, unemployment_rate DOUBLE NOT NULL, salary_value DOUBLE NOT NULL, salary_measure VARCHAR NOT NULL, salary_unit VARCHAR NOT NULL, salary_5y_growth DOUBLE NOT NULL,
        ...
        for _, r in merged.iterrows():
            rows.append([
                ...
                str(r.mapping_method_disco), str(r.mapping_source_disco), str(r.mapping_period_disco), str(r.mapping_confidence_disco),
                ...
                str(r.source_ai), str(r.dataset_ai), str(r.period_ai), str(r.source_url_ai), str(r.mapping_confidence_ai),
            ])
    ...
    (DATA_DIR / "education_profile_build_manifest.json").write_text(json.dumps(result, ensure_ascii=False, indent=2), encoding="utf-8")
